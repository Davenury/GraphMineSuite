...
def remove_vertexes(graph: Graph, to_remove: Graph) -> Graph:
    ...
    to_remove_vertex = VectorSetRDD.from_array(to_remove_ids)
...
def dir(graph: Graph) -> Graph:
    ...
    def normalize_edges(vertex: Set, vertex_id: int) -> Set:
        to_remove = [vertex for vertex in vertex.to_array()
                     if vertex <= vertex_id]
        to_remove_set = VectorSetRDD.from_array(to_remove)
...
def k_clique(graph: Graph, k: int):
    ...
    new_graph = degeneracy_order(graph, k)

    new_graph = dir(new_graph)
