...
echo $cluster_id
aws emr add-steps --cluster-id $cluster_id --steps Type=spark,Name=GraphProcessing,Args=[--master,yarn,--conf,spark.yarn.submit.waitAppCompletion=true,--num-executors,3,--executor-cores,3,--packages,graphframes:graphframes:0.6.0-spark2.3-s_2.11,--executor-memory,10g,s3://$bucket/GMS/aws_run.py,s3://$bucket/]
